...
const Booking = ({ service }) => {
    ...
    const onSubmit = data => {
        fetch('https://young-sea-60382.herokuapp.com/services/booking', {
            method: 'POST',
            headers: {
        ...
        <div>
            <div className="mb-5 mt-5">
                <form className="flex flex-col w-80 mx-auto" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <input className="py-2 px-5 rounded-full mx-auto bg-pink-600 text-white text-xl font-semibold mt-3" type="submit" value="Order Now" />
                </form>
            </div>
        </div>
    );
